...
export default function App() {
  ...
  // Almanacco (Punto 1: Visualizzazione Diretta)
  const [almanacco, setAlmanacco] = useState({
    santo: "San Tommaso Becket", 
    proverbio: "San Tommaso, il freddo al naso.", 
    curiosita: "Il 29 dicembre 1170 Tommaso Becket fu assassinato nella cattedrale di Canterbury."
  });

  // Modal Impegni (Punto 3: 10 Campi)
  ...
  // Modal Liste (Punto 4: Titolo + Descrizione)
  const [showListModal, setShowListModal] = useState({ show: false, type: '', data: { titolo: '', descrizione: '' } });

  useEffect(() => {
    fetchMeds();
    // Logica Almanacco: aggiorna i testi in base alla data
    // (In un caso reale qui andrebbe la fetch a Wikipedia API)
  }, [selectedDate]);
      ...
      {showEventModal && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-xl rounded-[3rem] p-8 shadow-2xl overflow-y-auto max-h-[90vh]">
            <h2 className="text-lg font-black uppercase text-indigo-600 mb-6 italic">Nuovo Appuntamento</h2>
            ...
            <button onClick={() => {setGoogleEvents([...googleEvents, {...currentEvent, id:Date.now()}]); setShowEventModal(false);}} className="w-full bg-indigo-600 text-white font-black p-5 rounded-[2rem] mt-6 shadow-xl uppercase">Sincronizza e Salva</button>
          </div>
        </div>
      )}

      {/* MODAL LISTE (TODO/NOTE) */}
      {showListModal.show && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-8 shadow-2xl">
            <h2 className="font-black uppercase mb-6 text-gray-400 text-xs tracking-widest">{showListModal.type}</h2>
            <input type="text" placeholder="Titolo" className="w-full p-4 bg-gray-50 rounded-2xl mb-4 font-bold" value={showListModal.data.titolo} onChange={e=>setShowListModal({...showListModal, data:{...showListModal.data, titolo:e.target.value}})} />
            <textarea placeholder="Descrizione" className="w-full p-4 bg-gray-50 rounded-2xl mb-6" rows="4" value={showListModal.data.descrizione} onChange={e=>setShowListModal({...showListModal, data:{...showListModal.data, descrizione:e.target.value}})} />
            <button onClick={() => {
              const item = { ...showListModal.data, id: Date.now() };
              if(showListModal.type === 'todo') setTodoList([...todoList, item]);
              else setNotes([...notes, item]);
              setShowListModal({show:false});
            }} className="w-full bg-green-500 text-white font-black p-5 rounded-[2rem] uppercase shadow-lg">Aggiungi alla lista</button>
          </div>
        </div>
      )}
      ...
      <main className="p-6 space-y-8 max-w-2xl mx-auto">
        {activeTab === 'home' && (
          <div className="space-y-8 animate-in fade-in duration-500">
            {/* 1. ALMANACCO (TESTO DIRETTO) */}
            <section className="bg-white p-8 rounded-[3.5rem] shadow-sm border border-indigo-50 text-left">
              <h2 className="text-[10px] font-black uppercase text-indigo-500 mb-4 tracking-tighter">Almanacco 🗓️</h2>
              ...
              <div className="p-5 bg-indigo-50/50 rounded-[2.5rem] text-[11px] leading-relaxed text-gray-600">
                <span className="font-black text-indigo-600 uppercase block mb-1">Cosa accadde oggi:</span>
                {almanacco.curiosita}
              </div>
            </section>
            ...
            <section className="bg-white p-8 rounded-[3.5rem] shadow-sm border border-indigo-50">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-[10px] font-black uppercase text-indigo-500 italic">Impegni Google</h2>
              ...
              {googleEvents.filter(e => e.data === selectedDate).map(e => (
                <div key={e.id} className="p-6 bg-gray-50 rounded-[2.5rem] mb-3 border-l-[10px] flex justify-between items-center" style={{borderColor: e.colore}}>
                  <div><p className="text-[10px] font-black text-gray-400 uppercase">{e.oraInizio}</p><h4 className="font-black text-gray-800 uppercase italic">{e.titolo}</h4></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <section className="bg-white p-8 rounded-[3.5rem] shadow-sm border border-green-50">
                ...
                {todoList.map(t => <div key={t.id} className="p-3 bg-green-50/20 rounded-xl mb-2 flex justify-between"><div><p className="text-xs font-bold">{t.titolo}</p></div><button onClick={()=>setTodoList(todoList.filter(i=>i.id!==t.id))}>✕</button></div>)}
              </section>
              <section className="bg-white p-8 rounded-[3.5rem] shadow-sm border border-amber-50">
                ...
                {notes.map(n => <div key={n.id} className="p-3 bg-amber-50/20 rounded-xl mb-2 flex justify-between"><div><p className="text-xs font-bold">{n.titolo}</p></div><button onClick={()=>setNotes(notes.filter(i=>i.id!==n.id))}>✕</button></div>)}
              </section>
            </div>

            {/* 5. FARMACI (SINCRONIZZATI) */}
            <section className="bg-white p-8 rounded-[3.5rem] shadow-sm border border-red-50 text-left">
              <h2 className="text-[10px] font-black uppercase text-red-400 mb-6 italic">Farmaci di Oggi</h2>
              {meds.map(m => (
                <div key={m.id} className="flex items-center justify-between p-4 bg-red-50/20 rounded-[1.5rem] mb-3 border border-red-50/50">
                  <div><p className="font-black text-sm uppercase text-gray-700">{m.name}</p><p className="text-[9px] font-bold text-red-300 uppercase">{m.schedule_time} - {m.description}</p></div>
        ...
        {activeTab === 'agenda' && (
          <div className="space-y-6">
            <div className="flex bg-gray-200 p-1.5 rounded-[2.5rem] shadow-inner">
            ...
            {agendaSubTab === 'diario' ? (
              <section className="bg-white p-8 rounded-[3.5rem] shadow-sm border border-amber-100 text-left">
              ...
              <div className="bg-white p-8 rounded-[3.5rem] shadow-sm min-h-[400px]">
                <h3 className="text-xs font-black uppercase text-indigo-500 mb-6 text-center">Calendario Google Sincronizzato</h3>
                {/* Qui andrebbe l'iframe o la vista calendario */}
              </div>
            )}
          </div>
        )}
